  ...
  return <div className="space-y-6">
    ...
    <div className="flex flex-col sm:flex-row gap-3">
      <label className="relative flex-1">
        ...
        <input aria-label="Хичээл хайх" value={filters.search} onChange={e => setFilters({ ...filters, search: e.target.value, page: 1 })} placeholder="Нэр эсвэл кодоор хайх..." className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 dark:bg-slate-800" />
      </label>
      <input aria-label="Түвшин" value={filters.level} onChange={e => setFilters({ ...filters, level: e.target.value, page: 1 })} placeholder="Түвшин" className="px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 dark:bg-slate-800" />
    </div>
    {loading ? <LoadingCards /> : error ? <ErrorState error={error} onRetry={load}/> : result.items.length === 0 ? <ContextualEmpty title="Хичээл олдсонгүй" description={filters.search||filters.level?'Хайлтын нөхцөлөө цэвэрлээд дахин оролдоно уу.':'Таны бүртгэлтэй published хичээл одоогоор алга.'} action={(filters.search||filters.level)&&<button className="btn-primary" onClick={()=>setFilters({search:'',level:'',page:1,limit:9})}>Шүүлтүүр цэвэрлэх</button>} /> :
      <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-5">{result.items.map(course =>
        <Link key={course.id} to={`/student/courses/${course.id}`} className="group bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden hover:shadow-lg transition">
