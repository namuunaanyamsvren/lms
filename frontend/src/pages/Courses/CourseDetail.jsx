...
export default function CourseDetail() {
  ...
  const load = () => fetchCourseById(id).then(setCourse).catch(() => setError('Хичээл ачааллахад алдаа гарлаа.'));
  useEffect(() => {
    load();
  }, [id]);
  ...
  return <div className="max-w-5xl mx-auto space-y-6">
    <div className="rounded-2xl overflow-hidden bg-slate-900 text-white grid md:grid-cols-[1fr_300px]">
      <div className="p-8"><span className="text-indigo-300 text-sm">{course.code} · {course.credits} кредит · {course.durationWeeks ? `${course.durationWeeks} долоо хоног` : 'хугацаа тохируулаагүй'} · {Number(course.price || 0).toLocaleString('mn-MN')} {course.currency || 'MNT'}</span><h1 className="text-3xl font-bold mt-2">{course.title}</h1><p className="text-slate-300 mt-3">{course.description}</p></div>
    ...
    {course.billingRestricted && <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-amber-900">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="mt-1 text-sm">Энэ хичээлийн материал төлбөр баталгаажсаны дараа нээгдэнэ.</p>
        </div>
        <Link to="/student/payments" className="rounded-xl bg-amber-600 px-4 py-2 text-sm font-semibold text-white hover:bg-amber-700">Төлбөр харах</Link>
      </div>
    </div>}
    <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border"><div className="flex justify-between text-sm mb-2"><span>Нийт явц</span><b>{course.progress || 0}%</b></div><div className="h-2 bg-slate-200 rounded"><div className="h-full bg-indigo-600 rounded" style={{ width: `${course.progress || 0}%` }} /></div></div>
    {!course.billingRestricted && course.modules.map(module => <section key={module.id} className="bg-white dark:bg-slate-800 rounded-xl border overflow-hidden"><h2 className="font-semibold p-4 border-b">{module.title}</h2>
