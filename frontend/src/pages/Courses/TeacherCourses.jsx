    ...
    <div className="grid gap-4 lg:grid-cols-[280px_1fr]">
      <aside className="rounded-xl border border-slate-200 bg-white p-4">
        <label className="relative block">
          ...
          <input className="w-full rounded-xl border border-slate-200 py-2.5 pl-10 pr-4 text-sm" value={search} onChange={e => setSearch(e.target.value)} placeholder="Хичээл хайх..." />
        </label>
        <div className="mt-3 space-y-2">
          {courses.map(course => (
            <button
              ...
              className={`w-full rounded-lg border px-3 py-2 text-left text-xs transition ${selectedCourse?.id === course.id ? 'border-indigo-300 bg-indigo-50 text-indigo-800' : 'border-slate-200 bg-slate-50 text-slate-700 hover:bg-slate-100'}`}
            >
          ...
          <div className="space-y-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <h2 className="mt-1 text-xl font-bold text-slate-900">{selectedCourse.title}</h2>
                  ...
                  {selectedStudents.length === 0 ? (
                    ...
                    return (
                      <tr key={student.id} className="bg-white">
                        ...
                        <td className="px-4 py-3 text-slate-600">{cohortNames || '—'}</td>
                      </tr>
                    );
                  })}
                </tbody>
            ...
            <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
              {courseActions.map(({ label, href, icon: Icon }) => (
                <Link
                  ...
                  className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-700 hover:border-indigo-200 hover:bg-indigo-50 hover:text-indigo-700"
                >
                  <Icon size={15} />
                  <span>{label}</span>
                </Link>
              ))}
              <Link to={`/teacher/courses/${selectedCourse.id}/builder`} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2 text-xs font-semibold text-indigo-700 hover:bg-indigo-100">
                <Edit3 size={15} />
                <span>Builder</span>
              </Link>
            </div>
