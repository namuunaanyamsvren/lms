...
router.post('/student-access-requests',requireRole('USER','PARENT','STUDENT'),(req,_res,next)=>{const result=z.object({organizationId:z.string().trim().min(1).max(100).optional(),requestedRole:z.enum(['STUDENT','PARENT','INSTRUCTOR','STAFF']).default('STUDENT'),guardianLinkCode:z.string().trim().min(4).max(50).optional(),note:z.string().trim().max(1000).optional()}).strict().superRefine((value,ctx)=>{if(value.requestedRole==='PARENT'&&!value.guardianLinkCode)ctx.addIssue({code:z.ZodIssueCode.custom,path:['guardianLinkCode'],message:'guardianLinkCode is required for parent access requests'});}).safeParse(req.body);if(!result.success)return next(AppError.badRequest('Invalid student access request',result.error.flatten()));req.body=result.data;next();},asyncHandler(requestStudentAccess));
