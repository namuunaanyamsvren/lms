...
| Latest PR failure: gitleaks false positive | PASS | GitHub log: `WEB_PUSH_PROVIDER_TOKEN=""` in `backend/notification-service/.env.example`, rule `generic-api-key`. Энэ нь хоосон example placeholder, genuine secret биш. Fix: `.gitleaks.toml` дээр зөвхөн `.env.example` файлын хоосон `*PROVIDER_TOKEN=""` мөрүүдэд scoped allowlist нэмсэн; real secrets/general tokens scan хэвээр. |
