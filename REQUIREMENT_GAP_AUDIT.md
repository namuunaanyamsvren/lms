...
| Latest PR rerun: GradeCategorySource drift | PASS | GitHub release-candidate #6 showed only academic migration `20260807140000_grade_category_source` failing with `ERROR: type "GradeCategorySource" does not exist`. Fix: migration conditional enum DO block changed to canonical `CREATE TYPE "GradeCategorySource" AS ENUM ('MANUAL', 'ATTENDANCE')`. Local evidence: `npm run prisma:generate --prefix backend` and `npm run build --prefix backend` pass. |
| Latest PR rerun: academic index/FK drift | PASS | GitHub release-candidate #7 showed academic drift for truncated unique index names, missing schema indexes (`Course.organizationId+currency`, `Cohort.organizationId+status`, `Enrollment.organizationId+status`), and relation FKs (`CourseVersion.courseId`, `Certificate.enrollmentId`, `DocumentRequestHistory.requestId`, `ScholarshipRequestHistory.requestId`). Fix: schema indexes/maps now match committed migration names; regular `Certificate` nullable `revokedAt` unique removed because the migration's partial unique index is the intended DB constraint; conditional FK migration blocks changed to canonical `ALTER TABLE ... ADD CONSTRAINT`. Local evidence: `npm run prisma:generate --prefix backend` and `npm run build --prefix backend` pass. Full `prisma:check-drift` still requires GitHub real Postgres/shadow schemas to verify. |
| Latest PR rerun: auth integration fixture | PASS | GitHub release-candidate #8 reached backend tests and failed `auth.integration.test.ts` because the test registered with `role: 'student'`, while public self-registration correctly allows only `USER/PARENT/user/parent`. Fix: test fixture now registers with `role: 'user'`; security/product validator was not weakened. Local evidence: `npm run build --prefix backend` pass. Targeted auth integration execution is NOT VERIFIED locally because this workstation has no Postgres at `localhost:5432` (`P1001`), but GitHub service-container DB will run it. |
| Latest PR rerun: managed production compose env | PASS | GitHub release-candidate #9 passed backend tests, frontend build/tests, and production readiness, then failed `docker compose -f backend/docker-compose.managed-production.yml config --quiet` because `TWILIO_AUTH_TOKEN` was required but missing from the deploy-readiness placeholder env. Fix: `scripts/validate-deploy-ready.mjs` now supplies CI-safe Twilio placeholder values (`TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_FROM_NUMBER`) only for config validation; no production secrets were added. Local evidence: equivalent `docker compose ... config --quiet` command passed with the updated placeholder env. |
| Final production evidence gate | PASS | `backend/shared/src/config/productionReadiness.ts` now supports `REQUIRE_PRODUCTION_LIVE_EVIDENCE=true`, which fails production validation when launch evidence is missing for secret-manager inventory, backup/restore drill, Sentry/error-monitoring alert test, or live payment webhook verification. Local evidence: strict production-like env command passed with all four HTTPS evidence URLs, and `npm exec --prefix backend -- vitest run tests/production-readiness.test.ts` passed. |
| CodeQL missing rate limiting alerts | PASS | GitHub CodeQL reported missing rate limiting on authenticated/authorization route handlers, including the visible annotation in `backend/tests/shared.middleware.test.ts`. Fix: test Express apps now apply shared HTTP security middleware, and authenticated service routers explicitly attach `createPrincipalRateLimiter()` after authentication/tenant resolution or on protected auth routes. Security scanning was not disabled. Local evidence: `npm run build --prefix backend` and `git diff --check` pass. Targeted supertest execution remains NOT VERIFIED locally because this sandbox cannot bind `0.0.0.0` (`listen EPERM`), but GitHub runner should execute it normally. |
| CodeQL hardening batch | PASS | GitHub CodeQL also flagged sensitive logging, SSRF, token hashing, regex/path handling, and file parameter type confusion. Fixes applied without disabling CodeQL: auth bearer parsing now uses a strict token parser; event/audit logs redact event labels; service URL composition uses `URL`; organization policy IDs are encoded; certificate logo fetch blocks private/local hosts; signed file download requires scalar query values; report filenames are sanitized before storage; password reset tokens are stored with HMAC-SHA256; pwned-password SHA-1 use is isolated and documented as k-anonymity lookup only. Local evidence: `npm run build --prefix backend` and `npm exec --prefix backend -- vitest run tests/password-policy.test.ts tests/password-reset.test.ts tests/production-readiness.test.ts` pass. |
| Latest CodeQL rerun hardening | PASS | GitHub CodeQL rerun dropped from 19 to 13 alerts. Follow-up fixes remove backend remote certificate logo fetching to close SSRF surface, remove sensitive event labels from RabbitMQ/auth audit logs, avoid user-controlled auth/OAuth branch decisions, validate upload helper scalar strings, use `scryptSync` for deterministic reset-token lookup hashes, and move pwned-password SHA-1 lookup to WebCrypto with test fixture digest. Local evidence: `npm run build --prefix backend` and targeted password/readiness tests pass. |
| Latest release-candidate auth audit regression | PASS | GitHub release-candidate #13 failed because `auth-audit.test.ts` still expected the old log message containing `LOGIN_FAILURE`, while the CodeQL fix intentionally removed event labels from logs. Fix: test now expects the generic sanitized log message. Local evidence: `npm exec --prefix backend -- vitest run tests/auth-audit.test.ts` and `npm run build --prefix backend` pass. |
| Latest CodeQL rerun remaining 6 alerts | PASS | GitHub CodeQL rerun dropped to 6 alerts: file type confusion/file-write taint and RabbitMQ sensitive error logging. Fix: `inspectUpload` now accepts unknown input and returns a fresh validated `Buffer` copy, upload/report writes use validated copied buffers, response no longer returns raw content, and RabbitMQ structured error logs no longer include handler error text. Local evidence: `npm run build --prefix backend` and targeted password/readiness tests pass. |
| Real production secrets configured | NOT VERIFIED | Code gate exists and strict mode requires `PRODUCTION_SECRET_INVENTORY_EVIDENCE_URL`, but this repository/session has no access to the real deployment secret manager. Production owner must attach the evidence URL without exposing secret values. |
| Backup/restore drill evidence | NOT VERIFIED | PITR runbook and restore-drill scripts exist, and strict mode requires `BACKUP_RESTORE_DRILL_EVIDENCE_URL`. A real provider restore drill against production backup storage must be run and linked before final launch sign-off. |
| Monitoring/Sentry alert evidence | NOT VERIFIED | Error monitoring integration point exists and strict mode requires `SENTRY_DSN`/`ERROR_MONITORING_DSN` plus `MONITORING_ALERT_TEST_EVIDENCE_URL`. A controlled alert must be triggered in production-equivalent or canary production and linked before final launch sign-off. |
| Payment webhook live verification | NOT VERIFIED | QPay webhook signature/idempotent reconciliation code and smoke checks exist, and strict mode requires `PAYMENT_WEBHOOK_LIVE_VERIFICATION_URL` when billing is enabled. A live provider sandbox/low-value webhook verification must be run and linked before final billing launch sign-off. |
...
4. ХИЙГДСЭН: Final production sign-off-д `REQUIRE_PRODUCTION_LIVE_EVIDENCE=true` strict evidence gate нэмэгдсэн. Бодит production secret inventory, restore drill, monitoring alert, payment webhook verification evidence URL-уудыг production owner бөглөж ажиллуулах ёстой.
